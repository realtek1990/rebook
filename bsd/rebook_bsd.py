"""ReBook for BSD (FreeBSD / OpenBSD / NetBSD / DragonFlyBSD) — GUI with CustomTkinter.
...
BSD port: uses sysctl for RAM detection, POSIX-compatible paths.
"""
...
def _find_marker_bin():
    """Find marker_single binary on BSD — checks venv, PATH, and local bin."""
    import shutil
    candidates = [
...
def _get_system_ram_gb():
    """Return total physical RAM in GB (BSD: sysctl hw.physmem)."""
    try:
        import subprocess
        ...
        pass
    # Fallback: try /proc/meminfo (some BSDs with procfs)
    try:
