...
def generate_sample(voice: str, callback: Callable[[str | None], None]):
    ...
    def _run():
        try:
            import edge_tts, subprocess, tempfile
...
def generate_audiobook(
    ...
    """
    Generate audiobook MP3s from an EPUB file.

    Args:
        epub_path: Path to .epub file
        voice: Edge TTS voice ID (e.g. "pl-PL-MarekNeural")
        output_dir: Directory where MP3s will be saved
        progress_cb: Called with (current, total, message)
    ...
    for i, chapter in enumerate(chapters):
        if progress_cb:
            progress_cb(i, total, f"Generuję: {chapter.title[:50]}…")
        ...
        # edge-tts has a character limit per request, chunk if needed
        chunks = _chunk_text(clean_text, max_chars=4500)
        if len(chunks) == 1:
            asyncio.run(_generate_chapter_async(clean_text, voice, out_path))
        else:
            ...
            for j, chunk in enumerate(chunks):
                tmp = tempfile.mktemp(suffix=".mp3")
                asyncio.run(_generate_chapter_async(chunk, voice, tmp))
    ...
    if progress_cb:
        progress_cb(total, total, f"Gotowe! {len(generated)} rozdziałów.")
